...
* [Prerequisites](https://docs.nginx.com/nginx/deployment-guides/setting-up-nginx-demo-environment/#prereqs)
* [Configuring NGINX Open Source for Web Serving](https://docs.nginx.com/nginx/deployment-guides/setting-up-nginx-demo-environment/#nginx-oss)
* [Configuring NGINX Plus for Load Balancing](https://docs.nginx.com/nginx/deployment-guides/setting-up-nginx-demo-environment/#nginx-plus)

### Prerequisites

This guide assumes you have already provisioned number of host systems \(physical servers, virtual machines, containers, or cloud instances\) required for a deployment guide \(if applicable\), and installed NGINX Open Source or NGINX Plus on each instance as appropriate. For installation instructions, see the [NGINX Plus Admin Guide](https://docs.nginx.com/nginx/admin-guide/installing-nginx/).
...
The steps in this section configure an NGINX Open Source instance as a web server to return a page like the following, which specifies the server name, address, and other information. The page is defined in the demo-index.html configuration file that you create in Step 4 below.
...
If you are using these instructions to satisfy the prequisites for one of our deployment guides, the Appendix in the guide specifies the name of each NGINX Open Source instance and whether to configure **App 1** or **App 2**.

**Note:** Some commands require `root` privilege. If appropriate for your environment, prefix commands with the `sudo` command.

1. Open a connection to the NGINX Open Source instance and change directory to **/etc/nginx/conf.d**:
...
3. Create a new file called **app.conf** with the following contents.
   ...
   Directive documentation: [`error_log`](https://nginx.org/en/docs/ngx_core_module.html#error_log), [`expires`](https://nginx.org/en/docs/http/ngx_http_headers_module.html#expires), [`index`](https://nginx.org/en/docs/http/ngx_http_index_module.html#index), [`listen`](https://nginx.org/en/docs/http/ngx_http_core_module.html#listen), [`root`](https://nginx.org/en/docs/http/ngx_http_core_module.html#root), [`server`](https://nginx.org/en/docs/http/ngx_http_core_module.html#server), [`server_name`](https://nginx.org/en/docs/http/ngx_http_core_module.html#server_name), [`sub_filter`](https://nginx.org/en/docs/http/ngx_http_sub_module.html#sub_filter)
...
The steps in this section configure an NGINX Plus instance to load balance requests across the group of NGINX Open Source web servers you configured in the [previous section](https://docs.nginx.com/nginx/deployment-guides/setting-up-nginx-demo-environment/#nginx-oss).

If you are using these instructions to satisfy the prequisites for one of our deployment guides, the Appendix in the guide specifies the names of the NGINX Plus instances used in the guide.

Repeat these instructions on each instance. As an alternative, you can configure one instance and share the configuration with its peers in a cluster. See the [NGINX Plus Admin Guide](https://docs.nginx.com/nginx/admin-guide/high-availability/configuration-sharing/).

1. Open a connection to the NGINX Plus instance and change directory to **/etc/nginx/conf.d**:
